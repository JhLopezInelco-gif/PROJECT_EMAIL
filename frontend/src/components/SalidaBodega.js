...
const CAMPOS = [
  ...
  { key: 'periferico', label: 'Periferico', type: 'text' },
  { key: 'marca', label: 'Marca', type: 'text' },
  { key: 'modelo_serie', label: 'Modelo/Serie', type: 'text' },
  { key: 'asignado', label: 'Asignado', type: 'text' },
  { key: 'retirado_por', label: 'Retirado Por', type: 'text' },
];
...
export default function SalidaBodega() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await inventarioAPI.salidaBodega.list({ page, page_size: pageSize, search: search || undefined });
      setItems(res.items);
      setTotal(res.total);
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) {
        await inventarioAPI.salidaBodega.update(editing, payload);
        toast.success('Salida de Bodega actualizada');
      } else {
        await inventarioAPI.salidaBodega.create(payload);
        toast.success('Salida de Bodega creada');
      }
      setShowModal(false);
      loadData();
    } catch (err) {
      toast.error('Error al guardar Salida de Bodega');
    }
  };

  const handleDelete = async (id) => {
    try {
      await inventarioAPI.salidaBodega.delete(id);
      toast.success('Salida de Bodega eliminada');
      setShowDeleteConfirm(null);
      loadData();
    } catch (err) {
      toast.error('Error al eliminar Salida de Bodega');
    }
  };
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4><i className="bi bi-box-arrow-up me-2"></i>Salida de Bodega</h4>
        <button className="btn btn-primary" onClick={openCreate}>
          <i className="bi bi-plus-circle me-2"></i>Nueva Salida
        </button>
      </div>
        ...
        <div className="card-body">
          <div className="row g-2 align-items-center">
            <div className="col-md-6">
              <input type="text" className="form-control" placeholder="Buscar salidas de bodega..."
        ...
        <div className="card">
          <div className="table-responsive">
            <table className="table table-hover table-striped mb-0">
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan={CAMPOS.length + 2} className="text-center text-muted py-4">No hay salidas de bodega registradas</td></tr>
                ) : items.map((item, idx) => (
                  <tr key={item.id}>
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{editing ? 'Editar Salida de Bodega' : 'Nueva Salida de Bodega'}</h5>
        ...
        <div className="modal show d-block" tabIndex="-1" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>¿Está seguro que desea eliminar la memoria <strong>{showDeleteConfirm.periferico} - {showDeleteConfirm.marca}</strong>?</p>
